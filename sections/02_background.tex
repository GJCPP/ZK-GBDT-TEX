...
Throughout the paper, we assume the encoding of a fixed-point number
    is in range $[-\mathcal{B}, \mathcal{B})$ for some public $\mathcal{B}>0$.
...
In our construction, we need a variant of this argument,
    ...
    \Cref{app:batched-lookup} gives the details of this argument and its properties.
...
we assume the prover and the client will engage three phases:
\begin{itemize}
    \item \textbf{Committing phase.} The prover commits to the training dataset and the trained forest.
    
    \item \textbf{Forest phase.} The prover proves the well-formedness of the committed forest and the correctness of the training procedure. If the verifier is not concerned with the training process, the prover can prove only the well-formedness of the committed forest in this phase.
...
In the proof of training, the public statement consists of the parameters
...
A similar statement applies to the proof of inference.
