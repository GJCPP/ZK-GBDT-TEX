...
We rely on the following standard properties. First, honestly generated openings
and batched openings always verify. Second, commitments are binding: after a
commitment is published, the prover cannot later open it to a different value at
the same point. Third, batched verification is sound: a prover cannot convince
the verifier of incorrect evaluation claims, except with negligible probability.
...
of boosting rounds $T$, the learning rate $\mu$, the quantization parameters,
and the tree-shape policy as public parameters of the statement being proved.

\paragraph{Boosting view.}
...
where $\mathbf{S}^{(t)}[r,k]$ is the current score of sample $r$ for class $k$
before the trees of round $t$ are added. We initialize
$\mathbf{S}^{(0)}=\mathbf{0}$.
...
\end{equation}
For the multinomial logistic loss, the negative gradient with respect to the
score matrix is
...
\end{equation}
We refer to $\mathbf{G}^{(t)}$ as the pseudo-residual matrix. Thus, in round
$t$, the learner trains one regression tree for each class $k\in[K]$ to fit the
target vector $\mathbf{G}^{(t)}[\cdot,k]\in\mathbb{F}^{M}$.
...
Equivalently, during training, each node $u$ is associated with the set
...
Among all \emph{valid} candidates, namely those for which both
$L_{u,j,b}\neq\varnothing$ and $R_{u,j,b}\neq\varnothing$, the trainer selects a
...
adding the tree outputs with learning rate $\mu$:
...
We follow the fixed-point quantization approach used in prior works~\cite{sparrow24,zkboost26}. 
