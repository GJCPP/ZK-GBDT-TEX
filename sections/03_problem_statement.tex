...
batching tools that make the resulting checks compose succinctly. Exact
statements are deferred to Sections~4--5.
...
\begin{itemize}
  ...
  where an internal-node row is one-hot and points from a parent node to its
  left or right child, while leaf rows are zero;
...
training to propagate histogram statistics from leaves to internal nodes. We
assume a topological node order in which children have larger indices than
their parents, so the later propagation equations have a unique recursive
interpretation.
...
that sample. Uniqueness follows from forest well-formedness: leaf boxes within
one tree form a partition of the quantized input domain, so two distinct leaf
rows cannot both contain the same query. Finally, the fetched leaf scores are
aggregated back along the
