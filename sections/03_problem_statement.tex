...
\begin{enumerate}
  ...
  \item The hyper-box of left and right children are consistent with the parent and the split, i.e. for each internal node $u$ and feature $j$,
    \begin{align}
    \mathbf{N}^{\leftarrow}[u, j] - \mathbf{N}^{\leftarrow}[\mathbf{lc}[u], j] &= 0\land\\
    \mathbf{N}^{\rightarrow}[u, j] - \mathbf{N}^{\rightarrow}[\mathbf{rc}[u], j] &= 0\land\\
    \mathbf{E}[u, j](\mathbf{N}^{\rightarrow}[\mathbf{lc}[u], j] - \mathbf{N}^{\leftarrow}[\mathbf{rc}[u], j]) &= 0\land\\
    (1-\mathbf{E}[u, j])(\mathbf{N}^{\leftarrow}[u, j] - \mathbf{N}^{\leftarrow}[\mathbf{rc}[u], j]) &= 0\land\\
    (1-\mathbf{E}[u, j])(\mathbf{N}^{\rightarrow}[u, j] - \mathbf{N}^{\rightarrow}[\mathbf{lc}[u], j]) &= 0.
    \end{align}
