...
\begin{itemize}
  ...
  root matrix $\mathbf{Root}\in\{0,1\}^{N_{\mathrm{tot}}}$;
...
its one-hot label $\mathbf{Y}\in \{0, 1\}^{M\times K}$.
The prover then commits leaves reached by each sample in the dataset,
by committing
\[
\mathbf{Ind} \in
\{N_{\mathrm{int}},\ldots,N_{\mathrm{tot}}-1\}^{T\times K\times M},
\]
...
Initially, $\mathbf{G}[0,k,r]=\mathbf{Y}[r,k]$ for all $k$ and $r$.
...
\begin{enumerate}
  ...
  \item $\mathbf{G}[0, \cdot, \cdot]$ is set correctly from $\mathbf{Y}$,
    namely $\mathbf{G}[0,k,r]=\mathbf{Y}[r,k]$.
  ...
  \item For each round $t$, the score $\mathbf{S}[t, \cdot, \cdot]$ is updated correctly
    from the previous score state and the scores each sample obtained from the tree trained at round $t$.
  \item For each round $t<T-1$, $\mathbf{G}[t+1, \cdot, \cdot]$ is updated correctly from $\mathbf{Y}$ and $\mathbf{S}[t, \cdot, \cdot]$.
    This includes proving softmax over the second dimension of $\mathbf{S}$.
\end{enumerate}

Together, these conditions imply that the committed objects encode a valid
plaintext GBDT training trajectory across all rounds.
