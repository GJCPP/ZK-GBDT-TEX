...
$u \in \{1,\ldots,N_{\mathrm{tot}}\}$ everywhere. When a node identifier labels
...
\begin{itemize}
  ...
  whose internal-node entries record the one-based node identifiers of the left and
  ...
  $\boldsymbol{\Theta}\in\{1, 2,\dots, B\}^{N_{\mathrm{tot}}}$,
...
\subsection{Proof of Forest Well-formedness}
...
\begin{enumerate}
  ...
  \begin{align}
    \sum_{u=1}^{N_{\mathrm{tot}}} \mathbf{root}[u] = TK.
  \end{align}
  \end{enumerate}
...
\[
\mathbf{Ind} \in
\{N_{\mathrm{int}}+1,\ldots,N_{\mathrm{tot}}\}^{T\times K\times M},
\]
...
\subsection{Proving the Correctness of Split}
