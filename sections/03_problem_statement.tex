...
We call this \emph{domain-lifted batching}. It lets us merge linear claims
...
that position, not on any other entry of the vector. Typical examples in our setting
include lookup-style checks against one common table, range checks against one
public range table. If each local
instance is proved per PCS, the proof size scales with the number of
instances even though the check itself is always the same.
