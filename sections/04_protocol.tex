...
For each $i\in[m]$, let
\[
\mathbf{u}_i(X):=\bigl(u_{i,1}(X),\ldots,u_{i,c}(X)\bigr)
\]
be a bundle of $c$ polynomial encodings over $\mathbb{H}_{n_i}$, where each
$n_i$ is a power of two. For some local relation $\varphi$ on $c$ inputs,
suppose the target claim is that all $m$ pointwise relations
\[
\forall i \in [m],\ \forall a\in\mathbb{H}_{n_i}\quad
\varphi\!\left(
u_{i,1}(a),\ldots,u_{i,c}(a)
\right)=0.
\]
There exists an algorithm that combines these $m$ claims into one
check over one interleaved bundle
\[
\widehat{\mathbf{u}}(X)=
\bigl(\widehat{u}_1(X),\ldots,\widehat{u}_c(X)\bigr)
\]
over domain $\mathbb{H}_{\widehat{n}}$, using only domain lifts and
coordinatewise pairwise interleaving and introducing no new PCS commitment.
Using a bucketed merge process, it runs in $O(m\log \widehat{n})$ time. This
...
\begin{enumerate}
  ...
  \varphi\!\left(
  \widehat{u}_1(z),\ldots,\widehat{u}_c(z)
  \right)=0;
  \]
  ...
  \]
  \item any opening claim for $\widehat{\mathbf{u}}(X)$ is reduced to one
  opening for each original bundle $\mathbf{u}_i(X)$, and the verifier can
  reconstruct the virtual opening in $O(mc\log \widehat{n})$ field operations.
  Interleaving reduces the number of logical local-check instances and avoids
  introducing new committed witness columns, but it does not by itself collapse
  all base PCS openings.
\end{enumerate}
\end{theorem}

\begin{proof}
Start from the active multiset $\{(\mathbf{u}_i(X),n_i)\}_{i\in[m]}$.
...
\cref{def:pairwise-interleaving} on every coordinate of the bundle. After this
pass, every bucket contains at most one active pair, and the total size
$S:=\sum_i n_i$ is unchanged.
...
\[
\widehat{n}= 2n_s' = 2^{\lceil \log_2 S\rceil}.
\]
...
interleaved bundle reduces to one opening per original bundle, and the verifier
reconstructs the virtual value by traversing the merge tree and evaluating
public selectors, which costs $O(mc\log \widehat{n})$ field operations.
\end{proof}
