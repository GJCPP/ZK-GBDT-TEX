...
\end{align}
If $v_1(X),\ldots,v_k(X)$ encode these vectors over $\mathbb{H}_n$, the polynomial
$f(X)=\phi(v_1(X),\ldots,v_k(X))$ is virtual: opening $f(t)$ reduces to
opening the $v_j(t)$ values and evaluating $\phi$ locally. For example,
...
\end{align}
We prove this zero-check through the equivalent divisibility test
\begin{align}
    f(X)=q(X)\nu_n(X),
    \quad
    \nu_n(X)=X^n-1.
...
interleaving same-form instances. For two vectors
