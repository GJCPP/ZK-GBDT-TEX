...
Initially, $\mathbf{G}[0,k,r]=\mathbf{Y}[k,r]-\lfloor\mathbf{1}\cdot 2^q/K\rfloor$ for all $k$ and $r$.
...
\begin{enumerate}
  ...
  \item $\mathbf{G}[0, \cdot, \cdot]$ is set correctly from $\mathbf{Y}$,
    namely $\mathbf{G}[0,k,r]=\mathbf{Y}[k,r]-\lfloor\mathbf{1}\cdot 2^q/K\rfloor$.
...
If we denote $\mathbf{C}[i, j, k]$ the number of samples falling to node $i$ with $j$-th feature being $k$,
  and $\mathbf{V}[i, j, k]$ the fixed-point sum of residuals for samples falling to node $i$ with $j$-th feature being $k$,
  then the proof uses the following fixed-point encoding of \eqref{eq:split-objective}:
\begin{equation}
\Phi(u,j,b)
\coloneqq
\left\lfloor\frac{\left(\sum_{k=0}^{b-1} \mathbf{V}[u, j, k]\right)^2}{2^q\cdot\sum_{k=0}^{b-1} \mathbf{C}[u, j, k]}\right\rfloor
+
\left\lfloor\frac{\left(\sum_{k=b}^{B-1} \mathbf{V}[u, j, k]\right)^2}{2^q\cdot\sum_{k=b}^{B-1} \mathbf{C}[u, j, k]}\right\rfloor.
\end{equation}
We note that the numerator $\mathbf{V}$ is quantized fixed-point, while the denominator $\mathbf{C}$ is not.
Hence we multiply $2^q$ to the denominator to ensure the equation is sematically meaningful. 
