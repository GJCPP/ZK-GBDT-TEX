...
% The first handles heterogeneous low-degree algebraic identities whose natural domains
% differ. The second merges many same-form non-native predicate checks into one
...
% heterogeneous low-degree algebraic claims, and interleaving batching for
% aligned same-form non-native predicates.

\subsection{Domain-Lifting Batching}\label{app:domain-lifting-batch}

As explained earlier, most low-degree algebraic constraints in our construction
are first expressed as residual zero-checks over some evaluation domain
$\mathbb{H}_n$. The
...
The intended setting for interleaving batching is that many non-native
predicate instances share the same local rule and the same public parameters,
but arrive as separate
...
\subsection{Interleaving Batching}\label{app:interleaving-batching}
