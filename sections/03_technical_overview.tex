...
\begin{itemize}
  ...
  \item a depth indicator $\mathbf{depth}\in\{0,1,\ldots,D\}^{N_{\mathrm{tot}}}$, where $D$ is the maximum depth of the trees in the forest. This is used for proving the correctness of tree construction.
...
\begin{enumerate}
  \item Consistency between indicators, i.e.
  \begin{align}
    \mathbf{valid}=0 &\iff \mathbf{depth} \neq \mathbf{0},\\
...
Additionally, to prove the uniqueness of roots, we utilize
...
\paragraph{Complexity.}
All forest-structural checks are linear or local constraints over either
$N_{\mathrm{tot}}$ node rows or $N_{\mathrm{tot}}d$ node-feature rows. The
...
\]
Domain-lifting batching combines the zero-check and sum-check constraints, and
interleaving batching combines the local lookup and range-check instances.
Consequently, the verifier time and proof size are
...
Given $\mathbf{Ind}$ and $\mathbf{G}$,
  the prover needs to prove the correctness of $\mathbf{C}, \mathbf{V}\in \mathbb{F}^{N_{\mathrm{tot}}\times d\times B}$.
  ...
  \begin{align}
  \mathbf{C}[i, j, k] =&\ \mathbf{C}_{\mathrm{leaf}}[i, j, k]\cdot \mathbf{is\_leaf}[i] +\nonumber\\
    &(\mathbf{C}[\mathbf{lc}[i], j, k] + \mathbf{C}[\mathbf{rc}[i], j, k])\cdot (1- \mathbf{is\_leaf}[i]),\\
  \mathbf{V}[i, j, k] =&\ \mathbf{V}_{\mathrm{leaf}}[i, j, k]\cdot \mathbf{is\_leaf}[i] +\nonumber\\
    &(\mathbf{V}[\mathbf{lc}[i], j, k] + \mathbf{V}[\mathbf{rc}[i], j, k])\cdot (1- \mathbf{is\_leaf}[i]),
  \end{align}
...
However, we note that the problem of proving histogram is one
  ...
  \begin{align}
    \sum_{i\in [N_{\mathrm{tot}}dB]} \mathbf{C}_{\{N_{\mathrm{tot}}dB\}}[i]\cdot \alpha^i &= \sum_{t, k, r, j} \alpha^{\mathsf{ind}(t, k, r, j)}\cdot 1,\\
    \sum_{i\in [N_{\mathrm{tot}}dB]} \mathbf{V}_{\{N_{\mathrm{tot}}dB\}}[i]\cdot \alpha^i &= \sum_{t, k, r, j} \alpha^{\mathsf{ind}(t, k, r, j)}\cdot \mathbf{G}[t, k, r],
  \end{align}
  where $\alpha$ is a random challenge and $\mathsf{ind}(t, k, r, j)$ is the flattened index of the tuple $(t, k, r, j)$,
  namely
  \[\mathsf{ind}(t, k, r, j) = \mathbf{Ind}[t, k, r]\cdot dB + j\cdot B + \mathbf{D}[r, j].\]
...
We note that, since Aurora-style sum-check~\cite{aurora19}
follows a similar pattern of the above divisibility check,
we can also batch them.
See \Cref{app:domain-lifting-batch} for 
formal description.
...
\]
These two public sparse selectors encode $(1,0,1,0,\ldots)$ and $(0,1,0,1,\ldots)$ over $\mathbb{H}_{2n}$, respectively. So after shifting $\mathbf{g}$ by one position, every position of the larger vector stores one original entry.

If the two vectors have different lengths, we first lift the smaller one to the
larger domain by the substitution $X\mapsto X^r$, 
which replicates the vector $r$ times over
the $r\times$ larger domain, as stated in
...
We formally present the interleaving batching in \Cref{app:interleaving}.
